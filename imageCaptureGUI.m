...
% Default Filename - Can be over-written by user input
 ...
 % Needle Gauge - Helpful to keep track of files later on
  ...
  %displays when an image is saved
  ...
  needleGAEdit = uicontrol('Style', 'edit', ...
                          'Position', [190 140 30 25], ...
                          'String', '21','FontSize',11,'HorizontalAlignment','left');

  uicontrol('Style', 'text', ...
         'Position', [5 110 180 25], ...
         'String', 'Liquid Density (kg/m³):', ...
         'FontSize', 11, 'FontWeight', 'bold', 'HorizontalAlignment', 'right');
  
  liquidDensityEdit = uicontrol('Style', 'edit', ...
         'Position', [190 110 60 25], ...
         'String', '998', 'FontSize', 11);
  
  uicontrol('Style', 'text', ...
         'Position', [5 80 180 25], ...
         'String', 'Air Density (kg/m³):', ...
         'FontSize', 11, 'FontWeight', 'bold', 'HorizontalAlignment', 'right');
  
  airDensityEdit = uicontrol('Style', 'edit', ...
         'Position', [190 80 60 25], ...
         'String', '1.04', 'FontSize', 11);
...
function runAnalysisCallback(~, ~)
    ...
    rhoLiquid = str2double(get(liquidDensityEdit, 'String'));
    rhoAir    = str2double(get(airDensityEdit,    'String'));

    imgFolder = fullfile(pwd, 'Outputs', prefix, strcat(needleGa, 'Ga'), 'Images');

    if ~isfolder(imgFolder)
        errordlg(['No saved images found at: ' imgFolder], 'Folder Not Found');
        return
    end

    pendant_drop_analysis(imgFolder, str2double(needleGa), rhoLiquid, rhoAir);
end
